...
def create_targets_random(win,num_targets,blob_width):
    ...
    Uses degrees as input unit but converts the to pixels
    """
    targets = []
    ...
    min_size = 0.1 # standard min size
    if win.units == 'pix':
        blob_width = convert_deg_to_pix(blob_width)
        min_size = convert_deg_to_pix(0.1)
    ...
    for _ in range(num_targets):
        ...
        target = stimulus.make_gaussian_blob(win, blob_width,min_size)
        target.pos = [x,y]
...
def convert_deg_to_pix(deg,):
    # visual angle (deg) → cm on screen
    cm = 2 * pm.VIEW_DISTANCE * math.tan(math.radians(deg / 2))

    # cm → pixels
    pix_per_cm = pm.RESOLUTION_X/pm.SCREEN_WIDTH

    return cm * pix_per_cm
